...
const ContactMeModal = ({ openModal, setOpenModal }) => {
  ...
  const onSubmit = () => {
    ...
    send(process.env.NEXT_PUBLIC_SERVICE_ID, process.env.NEXT_PUBLIC_TEMPLATE_ID, resultsTosend, process.env.NEXT_PUBLIC_API_KEY)
      .then((response) => {
                      ...
                      <form onSubmit={handleSubmit(onSubmit)}>
                        <div className="gap-3 text-center sm:grid sm:grid-cols-2 flex flex-col">
                          <div>
                            ...
                            />
                            <p className="text-left text-red-500">{errors.firstName?.message}</p>
                          </div>
                          <div>
                            <input
                              {...register("lastName", {
                              ...
                              className="border-2 w-full "
                            />
                            <p className="text-left text-red-500">{errors.lastName?.message}</p>
                          </div>
                          <div>
                            <input
                              {...register("email", {
